...
-10之间的翻转是不需要重复计算的。 所以用Delta提前预处理一下。
...
for (k = i + 1; k < CITY_SIZE; k++)
{
Delta1[i][k] = calc_delta1(i, k, solution.permutation, cities);  // 去重操作
}
}
...
now_cost = inital_cost + Delta1[i][k];    //delta的结果和inital_cost的关系是什么
...
count++;
it++;
shaking(current_solution, cities);  //对当前解进行了扰动 ？？
